		...
		if (checkTime > CheckInterval){  
			...
			if(Input.GetMouseButtonDown(0)){ 
				...
				if(clickTime2 - clickTime1 < RepeatTime){ 
					clicked=true;
					OnDoubleClick(target,Input.mousePosition);
				} 
				if(!clicked){
					clicked=true;
					OnClick(target,Input.mousePosition);
				}
			...
			if(!clicked){
				if (Input.GetMouseButton(0)){
					...
					if(!dragging){
						...
						OnDragBegin(target,dragMousePosition);
					}else{
						//drag
						OnDrag(dragGameObject,Input.mousePosition-dragMousePosition);
					}
				}else if(dragging){
					//stop drag
					dragging=false;
					OnDragEnd(dragGameObject,Input.mousePosition);
